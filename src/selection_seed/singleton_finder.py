...
def singleton_reference_finder(
    ...
    singleton_reference_seqs_dict = myUtil.load_cache(
        options, "sng0_training_proteinIDs.pkl"
    )
    domain_score_limits = myUtil.load_cache(
        options, "sng0_training_proteinIDs_limits.pkl"
    )
    ...
    domain_score_limits, singleton_reference_seqs_dict = (
        get_singleton_reference_sequences(options, non_empty_keys)
    )

    # Logistic regression prediction of propability of presence of singleton for each genome
    predictions_all = predict_singleton_reference_seqs_for_each_domain(
    ...
    limits_dict, sng_reference_seq_dict = collect_predicted_singleton_hits_from_db(
        predictions_all, options.database_directory, plausible_cutoff=0.02
    )
...
def get_singleton_reference_sequences(
    options: Any, non_empty_keys: Set[str]
) -> Tuple[Dict[str, Dict[str, float]], Dict[str, Set[str]]]:
    """
    ...
    singleton_reference_seqs_dict = myUtil.load_cache(
        options, "sng_reference_seqs_dict.pkl"
    )

    if domain_score_limits and singleton_reference_seqs_dict:
        logger.debug(
            "Loaded existing reference sequences for genes without conserved genomic context"
        )
    ...
    with sqlite3.connect(options.database_directory) as con:
        ...
        for singleton in singletons:
            cur.execute(
                query, (singleton, 0.9)
            )  # HERE IS THE HARDCODE FOR BLAST SCORE RATIO CUTOFF
            hits = cur.fetchall()
