...
def initial_alignments(options: Any, fasta_output_directory: str) -> List[str]:
    ...
    logger.info(
        f"Trimming alignments with {options.gap_remove_threshold} column gap threshold"
    )
...
def align_fasta_with_mafft(input_fasta, output_fasta, cores=2):
    ...
    try:
        with open(output_fasta, "w") as output_file:
            ...
            subprocess.run(
                [mafft, "--thread", str(cores), "--auto", input_fasta],
                stdout=output_file,
...
def run_fasttree_on_alignment(alignment_file, cores=2):
    ...
    """
    # Find MAFFT executable
    veryfasttree = myUtil.find_executable(
        "VeryFastTree-avx2"
    )  # Ensure this function finds the MAFFT executable

    # Replace the original suffix with '.tree'
    base_name = os.path.splitext(alignment_file)[
        0
    ...
    try:
        ...
        print(
            "VeryFastTree-avx2 executable not found. Please make sure VeryFastTree-avx2 is installed and in your system's PATH or in the local bin directory."
        )
