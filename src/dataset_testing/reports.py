            ...
            if file_path.endswith(file_extension):
                ...
                with open(file_path, "r") as infile:
                    for line in infile:  # Make the fold matrices
                        ...
                        values = [
                            int(value.strip()) for value in line.strip().split("\t")
                        ]
                    ...
                    column_sums = [
                        sum(map(int, column)) for column in columns
...
def create_performance_file(
    options, performance_dict, directory, filename="/performance.txt"
):
    ...
    with open(file_path, "w") as file:
        ...
        sorted_items = sorted(
            ...
            key=lambda x: (x[0].split("_")[-1], x[1].mcc),
            reverse=True,
        )
...
def hit_id_reports(cv_directory, database, data):
    ...
    hit_scores = {
        label: defaultdict(lambda: float("inf")) for label in ["TP", "FP", "FN"]
    }

    for tp_dict, fp_dict, fn_dict in data:
        for dict_label, hit_dict in zip(
            ["TP", "FP", "FN"], [tp_dict, fp_dict, fn_dict]
        ):
    ...
    gene_vicinity, sorted_compositions = fetch_neighbouring_genes_with_domains(
        database, list(set.union(*[set(d.keys()) for d in key_counts.values()]))
    )
        ...
        sorted_data = sorted(
            (
                (
                    key,
                    score,
                    key_counts[dict_label][key],
                    gene_vicinity.get(key, [["No data"]]),
                )
...
def fetch_cluster_ids(db_path, protein_ids):
    ...
    with sqlite3.connect(db_path) as con:
        cur = con.cursor()
        cur.execute(
            f"SELECT proteinID, clusterID FROM Proteins WHERE proteinID IN ({','.join(['?'] * len(protein_ids))})",
            tuple(protein_ids),
        )
...
def fetch_cluster_proteins(db_path, cluster_ids):
    ...
    with sqlite3.connect(db_path) as con:
        cur = con.cursor()
        cur.execute(
            f"SELECT proteinID, clusterID FROM Proteins WHERE clusterID IN ({','.join(['?'] * len(cluster_ids))})",
            tuple(cluster_ids),
        )
...
def fetch_protein_domains(db_path, protein_ids):
    ...
    with sqlite3.connect(db_path) as con:
        cur = con.cursor()
        cur.execute(
            f"SELECT proteinID, domain FROM Domains WHERE proteinID IN ({','.join(['?'] * len(protein_ids))})",
            tuple(protein_ids),
        )
...
def fetch_neighbouring_genes_with_domains(database, protein_ids, num_workers=8):
    ...
    with Pool(num_workers) as pool:
        results = pool.starmap(
            fetch_cluster_ids, [(database, chunk) for chunk in protein_id_chunks]
        )
    ...
    with Pool(num_workers) as pool:
        results = pool.starmap(
            fetch_cluster_proteins, [(database, chunk) for chunk in cluster_id_chunks]
        )
    ...
    with Pool(num_workers) as pool:
        results = pool.starmap(
            fetch_protein_domains, [(database, chunk) for chunk in protein_id_chunks]
        )
    ...
    with Pool(num_workers) as pool:
        results = pool.starmap(
            fetch_gene_start_positions,
            [(database, chunk) for chunk in protein_id_chunks],
        )
    ...
    for protein_id in protein_ids:
        ...
        if cluster_id and cluster_id in cluster_to_proteins:
            ...
            protein_list = [
                (
                    p,
                    protein_to_start.get(p, float("inf")),
                    protein_to_domains.get(p, ["no_neighbours"]),
                )
    ...
    for cluster_id, protein_list in cluster_to_proteins.items():
        domain_composition = sorted(
            {
                tuple(sorted(protein_to_domains.get(protein, ["no_neighbours"])))
    ...
    sorted_compositions = sorted(
        cluster_compositions.items(), key=lambda x: x[1], reverse=True
    )
