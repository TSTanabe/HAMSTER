...
def initial_self_recognition_validation(options: Any) -> None:
    ...
    # Prepare shared data
    all_seq_number = count_fasta_headers(
        options.sequence_faa_file
    )  # Number of all TP + TN
    alignment_files = [
        os.path.join(options.fasta_output_directory, f)
        for f in os.listdir(options.fasta_output_directory)
        if f.endswith(".fasta_aln")
    ]
    ...
    args_list = [
        (options, alignment_file, all_seq_number, files_number)
    ...
    output_file = os.path.join(
        options.Hidden_markov_model_directory, "_ini_cutoffs.txt"
    )
    with open(output_file, "w") as out:
        out.write(
            "hmm_protein_name\toptimized_cutoff\ttrusted_cutoff\tnoise_cutoff\tMCC\n"
        )
...
def initial_training_data_performance(
    options: Any, alignment_file: str, all_sequence_number: int, files_number: int
    ...
    load = os.path.join(
        initial_validation_directory, f"{hmm_protein_name}.ini_cutoffs_pkl"
    )
    ...
    if not (
        hmm := os.path.join(
            options.Hidden_markov_model_directory, f"{hmm_protein_name}.hmm"
        )
    ) or not os.path.isfile(hmm):
        logger.warning(f"HMM file not found for {hmm_protein_name}: {hmm}")
        return
    # Define target file
    if not (
        sequence_faa_file := options.targeted_sequence_faa_file_dict.get(
            protein_name, options.sequence_faa_file
        )
    ):
    ...
    output_report = os.path.join(
        initial_validation_directory, f"{hmm_protein_name}.hmmreport"
    )
    ...
    optimized_cutoff, trusted_cutoff, noise_cutoff, report, best_MCC, matrix = cutoffs(
        TP_seqIDs, TN, output_report
    )

    # Save the labelling at the optimal position for the bigger report
    myUtil.save_cache(
        options,
        f"{hmm_protein_name}.ini_performance_pkl",
        report,
        initial_validation_directory,
    )
    myUtil.save_cache(
        options,
        f"{hmm_protein_name}.ini_cutoffs_pkl",
        (
            hmm_protein_name,
            optimized_cutoff,
            trusted_cutoff,
    ...
    logger.debug(
        f"Finished HMM {hmm_protein_name} optimized cutoff: {optimized_cutoff} trusted cutoff: {trusted_cutoff} noise cutoff: {noise_cutoff} and MCC: {best_MCC}"
    )
...
def parallel_cross_validation(options: Any) -> None:
    ...
    args_list = [
        (alignment_file, options, all_seq_number, files_number)
        for alignment_file in alignment_files
    ]
...
def process_cross_folds(
    alignment_file: str, options: Any, all_sequence_number: int, files_number: int
) -> None:
    """
    ...
    cv_directory, cv_subfolder_name = setup_cross_validation_directory(
        alignment_file, options.cross_validation_directory
    )

    logger.debug(f"Cross-validation of HMM {cv_subfolder_name}")
    # Check if the task was already completed
    if os.path.exists(
        os.path.join(cv_directory, f"{cv_subfolder_name}_cv_matrices.txt")
    ):
        return None
    ...
    process_cutoffs_and_save_results(
        alignment_file,
        cv_reports,
        all_sequence_number,
        cv_directory,
...
def setup_cross_validation_directory(
    alignment_file: str, cross_validation_directory: str
) -> Tuple[str, str]:
...
def process_cutoffs_and_save_results(
    ...
    for report in cv_reports:
        optimized_cutoff, trusted_cutoff, noise_cutoff, report, MCC, matrix = cutoffs(
            TP_seqIDs, TN, report
        )
        model_values.append(
            [optimized_cutoff, trusted_cutoff, noise_cutoff, MCC, matrix]
        )
...
def save_thresholds(
    cv_directory: str, cv_subfolder_name: str, strict_report: Any
) -> None:
    """Writes the calculated cutoff thresholds to a file."""
    with open(
        os.path.join(cv_directory, f"{cv_subfolder_name}_cv_thresholds.txt"), "w"
    ) as writer:
        writer.write(
            f"{cv_subfolder_name}\t{strict_report[2]}\t{strict_report[3]}\t{strict_report[4]}\n"
        )


def save_matrices(
    cv_directory: str, cv_subfolder_name: str, fold_matrix: List[List[int]]
) -> None:
    """Writes the confusion matrices to a file."""
    with open(
        os.path.join(cv_directory, f"{cv_subfolder_name}_cv_matrices.txt"), "w"
    ) as writer:
...
def create_cross_validation_sets(
    alignment_file: str, output_directory: str, ending: str = ".cv", num_folds: int = 5
) -> None:
    """
    ...
    for fold in range(num_folds):
        ...
        train_record_ids = (
            record_id_list[:start_idx] + record_id_list[end_idx:]
...
def HMMsearch(library: str, input_file: str, output_file: str, cores: int = 1) -> None:
    """
    ...
    status = subprocess.call(
        hmmsearch_cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
...
def calculateMetric(metric: str, TP: int, FP: int, FN: int, TN: int) -> float:
    """
...
def calculate_performance_and_sum_matrices(
    ...
    best_optimized_cutoff = (
        best_optimized_cutoff
        if best_optimized_cutoff and best_optimized_cutoff > 0
        else 10
    )
...
def safe_compare(mcc, best_mcc, optimized_cutoff, best_optimized_cutoff):
    if any(x is None for x in [mcc, best_mcc, optimized_cutoff, best_optimized_cutoff]):
        return False
    return mcc > best_mcc or (
        mcc == best_mcc and optimized_cutoff < best_optimized_cutoff
    )
