...
def process_initial_validations(
    ...
    cutoff_collection = _collect_cutoff_and_performance(
        pkl_files, init_val_dir, options
    )
...
def _collect_cutoff_and_performance(
    pkl_files: List[str], init_val_dir: str, options: Any
) -> Dict[str, Dict]:
    cutoffs = {}
    performance = {}
...
def _extract_cutoff_and_performance(
    pkl_file: str, init_val_dir: str, options: Any
) -> (str, Dict, Dict):
    ...
    cuts = {"optimized cutoff": opt, "trusted cutoff": trust, "noise cutoff": noise}
    perf = {"MCC": best_MCC, "Matrix [TP,FP,FN,TN]": best_matrix}
...
def write_pkl_tsv_reports(
    options: Any, db_path: str, collections: Dict, output_dir: str, pkl_files: List[str]
) -> None:
    """
...
def _add_neighborhood_info(
    df: pd.DataFrame, protein: str, options: Any, db_path: str
) -> pd.DataFrame:
    ...
    if missing:
        new_nb, _ = csb_proteins_selection.fetch_neighbouring_genes_with_domains(
            db_path, missing, options.sqlite_chunks
        )
...
def _skip_if_all_outputs_exist(
    ...
    confusion_files = [
        os.path.join(
            report_dir, f"neighborhood_confusion_{label.replace(' ', '_')}.tsv"
        )
...
def _plot_roc_and_mcc(
    df: pd.DataFrame, report_dir: str, protein: str, cutoffs: Dict
) -> None:
...
def _write_neighborhood_confusions(df, report_dir, cutoffs):
    for label, cutoff in cutoffs.items():
        write_neighborhood_confusion_data(df, cutoff, label, output_dir=report_dir)
...
def _write_mcl_plots_if_available(
    protein, report_dir, grp1_refs, grp2_refs, mcl1_cutoff, mcl2_cutoff, mcl_results
):
...
def write_roc_from_metrics_to_tsv(
    ...
    out_df = pd.DataFrame(
        {"FPR": fpr_vals, "TPR": tpr_vals, "Score": thresh_vals, "Label": labels}
    )
...
def write_neighborhood_confusion_data(
    df: pd.DataFrame,
    cutoff: float,
    ...
    df["_conf_label"] = np.where(
        preds_pos & true_pos,
        "TP",
        np.where(
            preds_pos & ~true_pos, "FP", np.where(~preds_pos & true_pos, "FN", "TN")
        ),
    )
...
def write_mcl_vs_references(
    ...
    if n_ref_total == 0:
        logger.warning(
            f"No reference sequences for {protein_type} in mcl vs reference printing"
        )
        return
...
def export_existing_mcc_curve(
    df: pd.DataFrame, score_col: str, mcc_col: str, output_path: str
) -> None:
    """
