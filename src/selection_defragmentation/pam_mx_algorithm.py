...
def train_logistic_from_pam_with_scores(
    ...
    for domain in domains:
        ...
        if y.nunique() <= 1:
            logger.debug(
                f"{domain} class distribution = {y.value_counts().to_dict()} not enough class variety"
            )
            continue  # skip domains with only 1 class

        # Exclude self-domain features to prevent leakage
        feature_cols = [
            col for col in combined_matrix.columns if not col.startswith(domain + "_")
        ]
...
def fetch_bsr_scores(
    database_path: str, grouped: Dict[str, Set[str]], chunk_size: int = 900
) -> Dict[str, float]:
    """
...
def build_presence_score_matrix(
    df: pd.DataFrame, hit_scores: Dict[str, float]
) -> pd.DataFrame:
    """
...
def create_presence_absence_matrix(
    ...
    chunk_args = [
        (database_directory, chunk) for chunk in chunked(all_protein_ids, chunk_size)
    ]
...
def write_pam_to_tsv(pam: Dict[str, Dict[str, List[str]]], output_path: str) -> None:
    """
...
def collect_predicted_singleton_hits_from_db_parallel(
    ...
    task_args = [
        (domain, genome_list, database_path, bsr_cutoff)
