...
def run_mmseqs_clustering(
    directory: str, min_seq_id: float, min_aln_len: float, cores: int
) -> Dict[str, str]:
    """
    ...
    for faa_file in faa_files:
        ...
        if os.path.exists(output_file_path):
            logger.debug(
                f"Linclust results already exist for {output_prefix} - skipping: {output_file_path}"
            )
...
def run_mmseqs_linclust_lowlevel(
    ...
    """
    logger.debug(
        f"Clustering sequences for at {float(min_seq_id) * 100} % identity with mmseqs linclust"
    )
    ...
    for faa_file in faa_files:
        ...
        logger.debug(
            f"Clustering sequences for {base} at {float(min_seq_id) * 100} % identity"
        )
...
def convert_linclust_to_mcl(input_tsv: str, output_txt: str) -> None:
    """
