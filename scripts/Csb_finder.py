        ...
        if (index <= len(proteinID_list) and index - 1 >= 0): #Check index bounds
            ...
            curr_protein.gene_start - prev_protein.gene_end <= distance:
                ...
                if new_sb:
                    ...
                    #print("added current and previous protein",curr_el_proteinID,"to synthenic block",cluster.get_clusterID())
                    new_sb = 0
                    ...
                    #print("added current protein",curr_el_proteinID,"to synthenic block",cluster.get_clusterID())
                    cluster.add_gene(curr_el_proteinID,curr_protein.get_domains(),curr_protein.gene_start,curr_protein.gene_end)
                    curr_protein.set_clusterID(cluster.get_clusterID())
            elif new_sb == 0:
                ...
                #print("new protein elemenet out of cluster range not adding ",curr_el_proteinID,"to synthenic block",cluster.get_clusterID())
                new_sb = 1
                clusterID_dict[f"{clusterID_name}_{clusterID_number}"] = cluster
                clusterID_number += 1
                cluster = Cluster(f"{clusterID_name}_{clusterID_number}",distance)
                #print("new synthenic block",cluster.get_clusterID())
...
#Algorithm that compares clusters and finds similar ones. return a hash, cluster type class => clusterID
#first a protein type query from a fasta file
#second retieve all relevant clusters, get all with protein type = X and clusterID not null,
# take all clusters
# apply synteny comparison search from csb finder
# return hash with classes

#den schwerpunkt im netz finden
#jeden mit jeden vergleichen mittels jaccard score
